...
#SBATCH --job-name=run_visualize
#SBATCH --output=run_visualize.out
#SBATCH --error=run_visualize.err
...
DESCRIPTOR="soap"             # Options: simple, soap, coulomb

# Option 1: Auto-detect mode (recommended)
# Set one or more formula directories, tool auto-detects stages
# (total/selected/relaxed) via remove/ and relaxed/ subdirectories.
CIF_DIRS="./Al13Sc3N16"            # Space-separated for multiple: "./Na2Cl2 ./Na2Cl3 ./Na2Cl5"
...
# Smart filtering based on RD workflow directory conventions:
#   --total-dir:  Collects all CIFs EXCLUDING relaxed/ subdirs (original structures)
#   --select-dir: Collects CIFs EXCLUDING remove/ and relaxed/ subdirs (selected only)
#   --relax-dir:  Collects CIFs ONLY from relaxed/ subdirs (optimized structures)

REDUCTION="tsne"                # Reduction method: umap, tsne
...
WORKERS=""                      # Parallel workers (optional)
